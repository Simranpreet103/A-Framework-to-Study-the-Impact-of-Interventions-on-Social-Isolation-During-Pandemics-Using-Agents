breed [ adults adult ]
breed [ elders elder ]
turtles-own [ neighbours s dist x y infected-time quarantine-time hospital-time ICU-time house place prob flag]
...
to move-adults
  if ticks != 0 [if ticks < 40 [ifelse remainder ticks 8 = 0 [if random-float 1 <= 0.9[ ifelse patch-here = house[][move-to house]]][rt random 360
    fd 400 + random 600]]]

  ifelse random-float 1 <= 0.8[if ticks >= 40[
    ifelse patch-here = house[][move-to house]]]
  [ifelse random-float 1 <= 0.5[if ticks >= 40[ask other turtles[if distance myself <= 2 [bk 2]]]][rt random 360
    fd 400 + random 600 ]]
  ;ifelse ticks > 40  [  if random-float 1 <= 0.8
  ;    [
   ;     ifelse patch-here = house [;;do nothing
    ;    ][move-to house]
    ;];]
 ; [ifelse ticks > 79 [if random-float 1 <= 0.5[ask other turtles[if distance myself <= 2 [bk 2]]]]
;[     rt random 360
;      fd 2.25 + random 1]]
    if pcolor = orange or pcolor = cyan [move-to one-of patches with [pcolor = black]  ]
...
to move-elders
  ...
  ;;      ] [ move-to house ]
   ; ]]
 ; [ifelse ticks > 79 [if random-float 1 <= 0.5[ask other turtles[if distance myself <= 2 [bk 2]]]][ rt random 360
;      fd 1 + random 1.375]]
    if (pcolor = orange)or (pcolor = cyan) [move-to one-of patches with [pcolor = black]  ]
...
to infect
  if (color = brown)
      [  ask other turtles-here [ if (color = pink) or (color = white) [if random-float 1 <= 0.8 [set color brown ]]]]
end
...
to count-neighbours-and-total-distance
  ask turtles
  [
         ifelse (color != magenta) and (color != lime) and (color != sky) and (color != yellow) [if ticks < 40 [
	ask other turtles
	[ if distance myself <= 20 [ ifelse random-float 1 <= prob [ set neighbours neighbours + 1
        set s s + distance myself][set neighbours neighbours
      set s s + 30]]]]
      if ticks >= 40  [ask other turtles
	[ if distance myself <= 10 [ ifelse random-float 1 >= 0.8 [ifelse random-float 1 <= prob [ set neighbours neighbours + 1
          set s s + distance myself][set neighbours neighbours
          set s s + distance myself]][set neighbours neighbours
          set s s + 30]]]]]
...
;  ask elders
 ; [
  ;  ifelse (color != magenta) and (color != lime) and (color != sky) [if ticks < 40 [
;	ask other turtles
;	[ if distance myself <= 20 [ if random-float 1 >= prob [ set neighbours neighbours + 1
 ;       set s s + distance myself]]]]
  ;    if ticks >= 40 [ask other turtles
	;[ if distance myself <= 10 [ if random-float 1 >= prob [ set neighbours neighbours + 1
   ;       set s s + distance myself]]]]]
    ;  [set neighbours neighbours
;set s s + 30]
;    ifelse (color != magenta) and (color != lime) and (color != sky) [set neighbours neighbours + count other turtles in-radius 20][set neighbours neighbours]
